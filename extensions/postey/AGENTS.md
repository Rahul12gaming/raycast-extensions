...
- Shared logic lives in `src/api.ts` (Postey client), `src/types.ts`, `src/constants.ts`, `src/utils.ts`, and `src/preferences.ts`.
- `assets/` stores `extension-icon.png`; `api.json` is the Postey OpenAPI spec; `package.json` is the Raycast manifest; `raycast-env.d.ts` provides types.
...
- Keep API payload keys aligned with Postey API expectations (snake_case like `draft_title`).
...
- Set the Postey API key in Raycast preferences; never commit secrets.
